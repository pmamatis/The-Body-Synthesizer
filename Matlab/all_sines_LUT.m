...
%% exponential LUT
Fmax = FandBS(length(FandBS),1);
FmaxToLog = log(Fmax);
x = linspace(0,1,1000);
LUT_EXP = exp(x*FmaxToLog)
% plot(LUT_EXP);
...
%write LUT_EXP
dlmwrite('sinLUT.c','const float LUT_EXP[] __attribute__ ((section(".rodata"))) = {','delimiter','', 'precision', precision,'-append')   
dlmwrite('sinLUT.c',LUT_EXP,'delimiter',',', 'precision', precision,'-append');  
dlmwrite('sinLUT.c','};','delimiter','', 'precision', precision,'-append');  
%% H-file
...
%EXP
dlmwrite('sinLUT.h',['const float LUT_EXP[',num2str(length(LUT_EXP)),'];'],'delimiter','', 'precision', precision,'-append'); 

...
HAL_StatusTypeDef SignalErzeugung_Init(TIM_HandleTypeDef htim, DAC_HandleTypeDef hdac){
	...
	for (int i =0; i < SUPPORT_POINTS; i++){
		...
		sinTable[i] = (double)(sinf_halbe_offset) + OFFSET;
		//Sägezahn
 ...
 */
uint16_t SignaleAddieren(uint8_t count, ...){
		...
		while(tmpCount--){
			...
			Signale.frequenz[tmpCount] = va_arg(argumentlist, unsigned int);
...
//									sinf0 = sinTable[Signale.frequenz[j]/Signale.frequenz[indexMin]];
									...
									break;
								case SAEGEZAHN:
									tmp = Signale.frequenz[j]/Signale.frequenz[indexMin]*i; //Sägezahnwert durch beschleunigen des Counters abhängig von Frequenz bestimmen
									...
									break;
								case DREIECK:
										addValueDreieck = addValueDreieck + dreieckTable[i*Signale.frequenz[j]/Signale.frequenz[indexMin]];//bereits erzeugtes Dreieck-Stützstellen-Array frequenzabängig durchgehen
									break;
								case PWM:
									//wird im gegensatz zu den anderen Signalen nur einmal addiert da es entweder Maximale Amplitude oder null ist
									if (PWMTable[i*Signale.frequenz[j]/Signale.frequenz[indexMin]]){
									addValuePWM = PWMTable[i*Signale.frequenz[j]/Signale.frequenz[indexMin]];//bereits erzeugtes PWM-Stützstellen-Array frequenzabängig durchgehen
									}
									break;
								default:
									return -1;
 ...
 */
HAL_StatusTypeDef Output_Signal(DAC_HandleTypeDef hdac, TIM_HandleTypeDef htim, uint16_t frequenz, uint8_t Signalart){
		...
		if (Signalart==ADDSIG)
			TimerIncrements = 108000000/(SUPPORT_POINTS_ADD_SIG*frequenz);
		else
			TimerIncrements = 108000000/(SUPPORT_POINTS*frequenz);

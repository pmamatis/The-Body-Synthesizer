...
float LFO_SingleValueProcess(struct effects_LFO* effect) {
	...
	switch(effect->quarter) {
	case 0:
		effect_LFO = LFO[effect->index];
		break;
	case 1:
		effect_LFO = LFO[LFO_ENDINDEX[0] - effect->index];
		break;
	case 2:
		effect_LFO = -LFO[effect->index];
		break;
	...
	effect->index = round((double)(effect->index + frequency_ratio));
